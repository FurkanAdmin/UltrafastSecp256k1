...
int main(int argc, char** argv) {
    ...
    // BENCH-003: randomize the libsecp256k1 context before signing so that
    // blinding is enabled, matching production usage. Without this call libsecp
    // signs without scalar blinding, which is measurably faster — creating an
    // asymmetric comparison where Ultra CT signing (always blinded) is unfairly
    // disadvantaged. A deterministic fixed seed is used so the benchmark remains
    // reproducible across runs while still exercising the blinded code path.
    {
        const unsigned char ls_ctx_seed[32] = {
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
            0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
            0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
            0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20,
        };
        if (!secp256k1_context_randomize(ls_ctx, ls_ctx_seed)) {
            printf("[WARN] secp256k1_context_randomize failed — libsecp signing "
                   "may run without blinding (benchmark comparison may be skewed)\n");
        }
    }
...
#pragma GCC diagnostic pop
    ...
    double u_frost_aggregate = 0;
    {
        ...
        {
            const std::size_t frost_bench_calls =
                static_cast<std::size_t>(H.warmup_iters) +
                (H.passes * static_cast<std::size_t>(N_SIGN));
            std::vector<FrostNonce> bench_nonces(frost_bench_calls);
            for (std::size_t i = 0; i < frost_bench_calls; ++i) {
                auto tseed = make_hash(0xBEEF0000ULL + static_cast<std::uint64_t>(i));
                bench_nonces[i] = frost_sign_nonce_gen(1, tseed).first;
            }
            std::size_t bn_i = 0;
            u_frost_sign_partial = bench_ns([&]{
                auto ps = frost_sign(kp1, bench_nonces[bn_i++],
                                     msghashes[0], ncomms);
    ...
    double u_musig2_sig_agg = 0;
    {
        ...
        {
            const std::size_t musig2_bench_calls =
                static_cast<std::size_t>(H.warmup_iters) +
                (H.passes * static_cast<std::size_t>(N_SIGN));
            std::vector<MuSig2SecNonce> m2_nonce_pool(musig2_bench_calls);
            for (std::size_t i = 0; i < musig2_bench_calls; ++i) {
            ...
            u_musig2_partial_sign = bench_ns([&]{
                auto s = musig2_partial_sign(m2_nonce_pool[m2_i++],
