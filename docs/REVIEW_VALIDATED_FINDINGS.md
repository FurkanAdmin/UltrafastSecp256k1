...
|----|-------|---------|-----|
| CAAS-FG-01 | 5 shim security PoCs run in no blocking job (advisory-only) | **REAL → fixed (deeper than reported)** | Confirmed: root `add_subdirectory(audit)` (L489) precedes `compat/libsecp256k1_shim` (L497), so the bare `if(TARGET secp256k1_shim)` guard around the `shim_exploit_test` registrations was always FALSE → the 5 standalones were never CTest targets (`ctest -N` empty). Registering them (guard → `OR SECP256K1_BUILD_SHIM`) exposed that **3 of the 5 no longer compile** against removed APIs: `legacy_capi_*` → removed `bindings/c_api/ultrafast_secp256k1.*`; `bchn_schnorr_strict` → removed BCH `secp256k1_schnorr_*`. **Resolution:** resurrected + now hard-gated `exploit_context_flag_bypass` (9/9) + `exploit_musig_unknown_signer` (10/10); **retired** the 3 obsolete (properties covered by current-API strict-parse + BIP-340 lift_x tests). gate.yml runs the 2 with `--no-tests=error`; new guard `ci/check_advisory_has_blocking_test.py`. |
| CAAS-FG-01-SYSTEMIC | (discovered) | **REAL — tracked, larger triage** | The same bare-`if(TARGET secp256k1_shim)` registration anti-pattern affects **~31** other shim-dependent audit standalones (e.g. `regression_shim_pubkey_sort` confirmed absent from `ctest -N`) — they too are silently un-registered. The new guard WARNS the count. Converting each needs per-test compile triage (most use the current shim API and should just register; a few may be bit-rotted like the 3 retired). **Not rushed** — flagged for a dedicated focused pass to avoid rushed retirements/build breaks. |
