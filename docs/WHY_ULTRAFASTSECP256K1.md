...
| Exploit tests | 260 PoC files, 269 registered modules, 0 failures |
...
| Exploit PoC tests | 269 dedicated adversarial PoC modules across 20+ coverage categories (`audit/test_exploit_*.cpp`) | 269 wired, 0 failures |
...
| **Total (audit runner)** | **unified_audit_runner** across 117 non-exploit modules + 269 exploit-PoC modules (379 total) | **~1,000,000+** |
...
All 117 non-exploit audit modules across all tested platforms return **AUDIT-READY**. Zero failures in the current CI run (not a lifetime claim — see `docs/AUDIT_COVERAGE.md` for current status).
