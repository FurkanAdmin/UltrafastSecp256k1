...
| Exploit tests | 261 PoC files, 273 registered modules, 0 failures |
...
| Exploit PoC tests | 273 dedicated adversarial PoC modules across 20+ coverage categories (`audit/test_exploit_*.cpp`) | 273 wired, 0 failures |
...
| **Total (audit runner)** | **unified_audit_runner** across 143 non-exploit modules + 273 exploit-PoC modules (416 total) | **~1,000,000+** |
| **Total (exploit PoC tests)** | **273 exploit-PoC modules** (261 source files — 9 modules share a file or use inline shim stubs) across 20+ coverage categories | **273 modules, 0 failures** |

All 143 non-exploit audit modules across all tested platforms return **AUDIT-READY**. Zero failures in the current CI run (not a lifetime claim — see `docs/AUDIT_COVERAGE.md` for current status).
All 273 exploit PoCs modules pass. Zero failures in the current CI run across all 20+ coverage categories.
