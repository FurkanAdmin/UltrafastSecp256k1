...
| Exploit tests | 260 PoC files, 268 registered modules, 0 failures |
...
| Exploit PoC tests | 265 dedicated adversarial PoC modules across 20+ coverage categories (`audit/test_exploit_*.cpp`) | 265 wired, 0 failures |
...
| **Total (audit runner)** | **unified_audit_runner** across 116 non-exploit modules + 268 exploit-PoC modules (379 total) | **~1,000,000+** |
| **Total (exploit PoC tests)** | **268 exploit-PoC modules** across 20+ coverage categories, all in `audit/test_exploit_*.cpp` | **265 modules, 0 failures** |

All 116 non-exploit audit modules across all tested platforms return **AUDIT-READY**. Zero failures in the current CI run (not a lifetime claim — see `docs/AUDIT_COVERAGE.md` for current status).
All 268 exploit PoCs modules pass. Zero failures in the current CI run across all 20+ coverage categories.
