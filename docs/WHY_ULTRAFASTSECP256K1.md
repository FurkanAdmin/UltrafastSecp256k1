...
| Exploit tests | 261 PoC files, 272 registered modules, 0 failures |
...
| Exploit PoC tests | 271 dedicated adversarial PoC modules across 20+ coverage categories (`audit/test_exploit_*.cpp`) | 271 wired, 0 failures |
...
| **Total (audit runner)** | **unified_audit_runner** across 132 non-exploit modules + 272 exploit-PoC modules (404 total) | **~1,000,000+** |
| **Total (exploit PoC tests)** | **272 exploit-PoC modules** (261 source files — 9 modules share a file or use inline shim stubs) across 20+ coverage categories | **272 modules, 0 failures** |

All 132 non-exploit audit modules across all tested platforms return **AUDIT-READY**. Zero failures in the current CI run (not a lifetime claim — see `docs/AUDIT_COVERAGE.md` for current status).
